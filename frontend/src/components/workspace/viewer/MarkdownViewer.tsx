...
const LazyMilkdownMarkdownEditor = lazy(async () => {
  const module = await import("./MilkdownMarkdownEditor");
  return { default: module.MilkdownMarkdownEditor };
});
...
export function MarkdownViewer({
  ...
  // Reset pending state when the underlying file changes.
  useEffect(() => {
    setError(null);
    ...
    <div className="flex h-full flex-col overflow-hidden rounded border border-neutral-200 bg-white dark:border-neutral-700 dark:bg-neutral-800">
      {canEdit ? (
        <div className="flex shrink-0 items-center justify-between gap-2 border-b border-neutral-200 px-3 py-2 dark:border-neutral-700">
          ...
          >
            {(["tiptap", "milkdown"] as const).map((value) => {
              const active = editorKind === value;
              return (
                <button
                  key={value}
                  type="button"
      ...
      {!canEdit ? (
        ...
        <MarkdownEditor
          key={`tiptap-${basePath ?? "unknown"}-${documentRevision}`}
          value={draft}
          onChange={setDraft}
        />
      ) : (
        ...
        >
          <LazyMilkdownMarkdownEditor
            key={`milkdown-${basePath ?? "unknown"}-${documentRevision}`}
            value={draft}
            onChange={setDraft}
          />
        </Suspense>
