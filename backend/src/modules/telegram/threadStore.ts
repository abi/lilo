...
const normalizeTelegramChatKey = (value: string): string => value.trim();

const buildKey = (channel: ChannelName, from: string, dayKey: string): string =>
  `${channel}:${normalizeTelegramChatKey(from)}:${dayKey}`;
...
const toDayKey = (now: Date, timeZone: string): string => {
  try {
    ...
    console.warn(
      `[telegram] Invalid workspace timezone ${JSON.stringify(timeZone)}; falling back to UTC`,
      error,
    );
    return new Intl.DateTimeFormat("en-CA", {
      timeZone: "UTC",
...
export const resolveDailyTelegramChatId = async (
  fromChatKey: string,
  now: Date,
  timeZone: string,
): Promise<string | null> => {
  const dayKey = toDayKey(now, timeZone);
  const key = buildKey("telegram", fromChatKey, dayKey);
  const map = await readThreadMap();
  return map[key]?.chatId ?? null;
};

export const storeDailyTelegramChatId = async (
  fromChatKey: string,
  now: Date,
  timeZone: string,
  chatId: string,
